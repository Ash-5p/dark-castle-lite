...
def select_name():
    ...
    while True:
        ...
        if len(player_name) == 0:  # Checks if input is empty
            ...
            center_print("Character name cannot be more than 15 characters long.\n")
            input()
            clear_terminal()
            select_name()
...
def select_character():
    ...
    for index, character in enumerate(characters, start=1): # Iterate through characters list and index number
        center_print(f"{index}) {character.name} (Might: {character.might} | Wisdom: {character.wisdom} | Cunning: {character.cunning})")
    ...
    while True:
        ...
        try:
            ...
            if 1 <= choice <= len(characters):
                ...
                center_print(f"Please enter a number between 1 and {len(characters)}.")
        except ValueError:
...
def run_game():
    ...
    # Chapter 3
    center_print("You find yourself in the middle landing of a winding staircase.\n")
...
def homescreen():
    ...
    clear_terminal()
    while True:
        center_print("Dark Castle lite\n")
        center_print("Play Game (p)")
        center_print("Instructions (i)")
        player_choice = input()
